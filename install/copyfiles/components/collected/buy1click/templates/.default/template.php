...
<?if($arResult["GOOD_SEND"]=="Y"):?>
	<?ShowMessage( array("MESSAGE"=>$arParams["COLLECT_MESSAGE_AGREE"],"TYPE"=>"OK") );?>
<?endif;?>
	...
	<?foreach($arResult["FIELDS"] as $arField):?>
		<?if($arField["SHOW"]=="Y"):?>
			<?if($arField["CONTROL_NAME"]!="RS_AUTHOR_ORDER_LIST"):?>
				...
				<?if(in_array($arField["CONTROL_NAME"], $arParams["REQUIRED_FIELDS"])):?><span class="collected_developkit_required">*</span><?endif;?>
				<input type="text" name="<?=$arField["CONTROL_NAME"]?>" value="<?=$arField["HTML_VALUE"]?>" /><br />
			<?else:?>
				<div class="collected_developkit_dontshow"><textarea name="<?=$arField["CONTROL_NAME"]?>"></textarea></div>
			<?endif;?>
		<?endif;?>
	<?endforeach;?>
	<?if($arParams["COLLECT_USE_CAPTCHA"]=="Y"):?>
		<?=GetMessage("MSG_CAPTHA")?><span class="collected_developkit_required">*</span>:<br />
