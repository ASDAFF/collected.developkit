...
$arParams["EVENT_TYPE"] = "COLLECTED_BUY_ONE_CLICK";
$arParams["REQUEST_PARAM_NAME"] = "collected_buy1click";
...
if(!function_exists('collected_add_buy1click_type'))
{
	function collected_add_buy1click_type($EVENT_TYPE)
	{
...
if($_REQUEST[$arParams["REQUEST_PARAM_NAME"]]=="Y" && class_exists(CEventType) && class_exists(CEventMessage))
{
	...
	if(!$arET = $rsET->Fetch())
	{
		collected_add_buy1click_type($arParams["EVENT_TYPE"]);
	}
	$arResult["LAST_ERROR"] = "";
	$arResult["GOOD_SEND"] = "";
	...
	if(check_bitrix_sessid() && (!isset($_REQUEST["PARAMS_HASH"]) || $arResult["PARAMS_HASH"] === $_REQUEST["PARAMS_HASH"]))
	{
		...
		{
			$arEventFields = array();
			$arEventFields["THEME"] = GetMessage("COLLECT_MSG_THEME");
			$arEventFields["EMAIL_TO"] = trim( ( $arParams["COLLECT_EMAIL_TO"] ) );
