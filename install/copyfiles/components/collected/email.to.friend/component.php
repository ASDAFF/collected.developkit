...
$arParams["EVENT_TYPE"] = "COLLECTED_EMAIL_TO_FRIEND";
$arParams["REQUEST_PARAM_NAME"] = "collected_email2friend";
...
$arFields = array(
	...
	array(
		...
		"VALUE" => ( $arParams['COLLECT_LINK']!='' ? $arParams['COLLECT_LINK'] : "http://".$_SERVER["HTTP_HOST"].$APPLICATION->GetCurPageParam() ),
		"HTML_VALUE" => "",
	),
);
$arResult["FIELDS"] = $arFields;

if(!function_exists('collected_add_email2friend_type'))
{
	function collected_add_email2friend_type($EVENT_TYPE)
	{
...
if($USER->IsAuthorized()) $arParams["COLLECT_USE_CAPTCHA"] = "N";
...
if($_REQUEST[$arParams["REQUEST_PARAM_NAME"]]=="Y" && class_exists(CEventType) && class_exists(CEventMessage))
{
	...
	if(!$arET = $rsET->Fetch())
	{
		collected_add_email2friend_type($arParams["EVENT_TYPE"]);
	}
	$arResult["LAST_ERROR"] = "";
	$arResult["GOOD_SEND"] = "";
	...
	if(check_bitrix_sessid() && (!isset($_REQUEST["PARAMS_HASH"]) || $arResult["PARAMS_HASH"] === $_REQUEST["PARAMS_HASH"]))
	{
		...
		{
			$arEventFields = array();
			$arEventFields["THEME"] = $arParams["COLLECT_MESSAGE_THEMES"];
			...
			foreach($arResult["FIELDS"] as $key => $arField)
			{
				...
				if($arField["EVENT_FIELD_NAME"]=="EMAIL_TO")
				{
					...
					{
						$arResult["LAST_ERROR"] = GetMessage("COLLECT_MSG_BAD_EMAIL_TO_CHECK");
					}
				}
